...
function AppContent() {
    ...
    const [paymentAuthorizationUrl, setPaymentAuthorizationUrl] = (0, react_1.useState)(null);
    const [paymentReference, setPaymentReference] = (0, react_1.useState)(null);
    ...
    async function fetchPublicDataBundle() {
    ...
    async function applyPublicBundle(bundle) {
    ...
    async function fetchPrivateDataBundle(activeSession) {
    ...
    function applyPrivateBundle(bundle) {
        ...
        setAddressInput((current) => current || bundle.addressData[0]?.fullAddress || bundle.profileData?.customerProfile?.defaultAddress || "");
    }
    const publicDataQuery = (0, react_query_1.useQuery)({
        queryKey: ["customer-public-data"],
        queryFn: fetchPublicDataBundle
    });
    const privateDataQuery = (0, react_query_1.useQuery)({
    ...
    async function refreshTrackedOrderStatus(orderId) {
        if (!session)
            return;
        ...
        const order = await request(`/orders/${orderId}`, {}, session);
    ...
    (0, react_1.useEffect)(() => {
        if (!session || !trackedOrder || activeScreen !== "confirmation" || paymentMethod === "CASH")
            return;
        const timer = setInterval(() => {
            void refreshTrackedOrderStatus(trackedOrder.id).catch((err) => setCheckoutStatus(err instanceof Error ? err.message : "Unable to refresh payment status."));
        }, 12000);
        return () => clearInterval(timer);
    ...
    async function initializePayment(orderId, amount) {
        if (!session)
            return null;
        const paymentResponse = await request("/payments/initialize", {
            method: "POST",
            body: JSON.stringify({
                orderId,
    ...
    async function openPaymentAuthorization(url) {
        const supported = await react_native_1.Linking.canOpenURL(url);
        if (!supported) {
    ...
    async function placeOrder() {
        ...
        try {
            ...
            if (paymentMethod === "CASH") {
                ...
                setCheckoutStatus(paymentMethod === "CARD"
    ...
    const trackingMapRegion = (0, react_1.useMemo)(() => buildMapRegion([
        {
